...
后台地址：<https://zhuyz.art/wayfind-admin/>。登录后台后可以新增、编辑、删除分类，以及新增、编辑、删除、排序和移动入口。入口的“后台备注”只在后台保存和显示，公开 API 不会返回它。右上角的个人设置可以修改密码，修改后会要求重新登录。保存后，公开页面刷新即可读取服务器上的最新数据。后台默认使用浅色主题，右下角按钮可以切换深色主题，选择会保存在当前浏览器中。
