...
一个纯静态的个人网址导航页。页面不需要后端，站点内容写在 `app.js`，公开页面只保留两类入口：中转站与其他。
...
站点数据在 `app.js` 顶部的 `LINKS` 中。每个入口包含名称、地址、分类、简介、状态和色调，按现有格式增删即可。分类只使用 `中转站` 和 `其他`。

站点不提供面向访客的编辑入口；需要让所有访问者看到某个站点时，请直接编辑 `LINKS` 后提交。
